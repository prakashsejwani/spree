class GoogleCheckoutNotificationController < ApplicationController
  ...
  def create
    ...
    puts "===========================#{params[:new_order_notification][:shopping_cart][:merchant_private_data][:order_number].strip.to_i}"
   ...
   #  @order.update_attribute(:user, current_user) if current_user 
    ...
    @order.update_attribute(:user, current_user) if current_user 
     @order.update_attribute(:user, current_user) if current_user
      ...
      @order.checkout.update_attribute(:email , checkout_info[:email])
